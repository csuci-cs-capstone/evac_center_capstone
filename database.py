...
#Many to many for Evaccenter -> evacuee_transfer
evaccenter_evacuee_transfers = db.Table('evaccenter_evacuee_transfers',
db.Column('evacuee_transfer.evacuee_transfer_id', db.Integer, db.ForeignKey('evacuee_transfer.evacuee_transfer_id'), primary_key=True),
db.Column('evaccenter.evaccenter_id', db.Integer, db.ForeignKey('evaccenter.evaccenter_id'), primary_key=True))

#Many to many for Employee -> evacuee_transfer
employee_evacuee_transfers = db.Table('employee_evacuee_transfer',
db.Column('evacuee_transfer.evacuee_transfer_id', db.Integer, db.ForeignKey('evacuee_transfer.evacuee_transfer_id'), primary_key=True),
db.Column('employeer.employee_id', db.Integer, db.ForeignKey('employee.employee_id'), primary_key=True))

#Many to many for Evacuee -> evacuee_transfer
Evacuee_Transfers = db.Table('Evacuee_transfers',
db.Column('evacuee_transfer.evacuee_transfer_id', db.Integer, db.ForeignKey('evacuee_transfer.evacuee_transfer_id'), primary_key=True),
db.Column('evacuee.evacuee_id', db.Integer, db.ForeignKey('evacuee.evacuee_id'), primary_key=True))
...
class Evaccenter(db.Model):
    ...
    evaccenter_evacuee_transfers = db.relationship('Evacuee_transfer', secondary=evaccenter_evacuee_transfers, lazy='subquery', backref=db.backref('evaccenters', lazy=True))
...
class Employee(db.Model):
    ...
    employee_evacuee_transfers = db.relationship('Evacuee_transfer', secondary=employee_evacuee_transfers, lazy='subquery', backref=db.backref('employees', lazy=True))
...
class Evacuee(db.Model):
    ...
    Evacuee_Transfers = db.relationship('Evacuee_transfer', secondary=Evacuee_Transfers, lazy='subquery', backref=db.backref('evacuee', lazy=True))
    # One-to-one relationships
...
class EvacueeTransfer(db.Model):
    ...
    def __repr__(self):
        return '<EvacueeTransfer %r>' % self.EvacueeTransfer_id, self.evac_id_1, self.evac_id_2, self.date_added
